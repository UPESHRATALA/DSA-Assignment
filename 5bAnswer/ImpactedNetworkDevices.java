...
public class ImpactedNetworkDevices {
  ...
  ImpactedNetworkDevices(int v) {
    this.v = v;
    network = new int[v][v];
  }

  // Function to add an edge between connected devices in the network (undirected
  // graph)
  ...
  public List<Integer> findImpactedDevices(int targetDevice, int source) {
    ...
    // Mark the source and its direct connections as visited to exclude them from
    // impacted devices
    visited[source] = true;
  ...
  private void dfsCheckSource(int node, boolean visited[], List<Integer> impactedDevices, int source,
    ...
    // If the node is not connected to the source, add it to the impactedDevices
    // list
  ...
  public static void main(String[] args) {
    ...
    ImpactedNetworkDevices isp = new ImpactedNetworkDevices(8);
